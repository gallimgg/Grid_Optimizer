...
def do_parallel_monte_carlo(
    ...
    def calculate_combined_lcoe(lcoe_values, dynamic_values, total_system_power):
        ...
        print(f"WHAT ACTUALLY GOES IN LCOE Total System Power {total_system_power}")
    ...
    for scc_case, social_cost_per_tonne in SOCIAL_COST_CARBON_CASES.items():
        ...
        for lca_method in ["constant_2025", "interpolated"]:
            ...
            baseline_vehicle_carbon_cost = (
                    vehicle_lca["baseline_ice_transport_co2_tonnes"]
                    * social_cost_per_tonne
            )

            modeled_vehicle_carbon_cost = (
                    vehicle_lca["modeled_transport_co2_tonnes"]
                    * social_cost_per_tonne
            )

            avoided_vehicle_carbon_cost = (
                    baseline_vehicle_carbon_cost
                    - modeled_vehicle_carbon_cost
            )
...
for reliability_label, min_pass_count in reliability_cases.items():
    ...
    for i in range(12, 41):
        year = 2022 + i - 12
        print(
            f"Running model for year: {year}, "
            f"case={reliability_label}, "
            f"NG_FACT={NG_Fact}"
        )
        ...
        while True:
            ...
            if results["status"] == "failure":
                ...
                print(
                    f"Year {year}, {reliability_label} failed "
                    f"({results['pass_count']}/10 passes). "
                    f"Reasons: {', '.join(failure_reasons)}. "
                    f"Adding resources."
                )

                Solar_Installed += 596 * 0.7
                Wind_Installed += (849 / 3200) * 1
                Baseload_Installed += 400
                ...
                print(
                    f"Year {year}, {reliability_label} succeeded "
                    f"with {results['pass_count']}/10 passes. "
                    f"Total Deficit: {results['total_energy_deficit']:.2f}"
                )

                if NG_Decrementor:
                    NG_Fact = max(NG_Fact - 0.0357, 0.0)
                ...
                if Nuke_Builder:
                    Baseload_Installed += 1

                output_yearly.append({
                    "Reliability_Case": reliability_label,
                    "Min_Pass_Count": min_pass_count,
    ...
    output_yearly_df = pd.concat(
        all_reliability_outputs.values(),
        ignore_index=True,
    )
...
# Use only the 10/10 reliability path for all main plots
main_reliability_label = "10_of_10"

output_yearly_df_all_cases = output_yearly_df.copy()

output_yearly_df = (
    output_yearly_df_all_cases
    .loc[output_yearly_df_all_cases["Reliability_Case"] == "10_of_10"]
    .copy()
    .sort_values("Year")
...
# -----------------------------
# Plot 2: Combined LCOE vs Year
# -----------------------------
fig, ax = plt.subplots(figsize=(12, 8))

for reliability_label, df_case in all_reliability_outputs.items():
    ax.plot(
        df_case["Year"],
        df_case["Combined_LCOE"],
        marker="o",
        linewidth=LINE_WIDTH,
        markersize=MARKER_SIZE,
        label=f"{reliability_label} Combined LCOE"
    )
...
ax.set_title(
    "Combined LCOE by Reliability Requirement",
    fontsize=TITLE_SIZE,
    fontweight=FONT_WEIGHT,
)
...
plt.tight_layout()
lcoe_reliability_plot_path = f"{folder_name}/lcoe_by_reliability.png"
plt.savefig(lcoe_reliability_plot_path, dpi=300, bbox_inches="tight")
plt.show()
plt.close(fig)
