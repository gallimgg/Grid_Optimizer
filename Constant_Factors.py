...
# Calculate the combined LCOE
def calculate_combined_lcoe(energy_sources, dynamic_values, total_system_power):
    """
    Calculate the combined LCOE for all energy sources.

    :param energy_sources: Dictionary of financial parameters for all energy sources
    :param dynamic_values: Dictionary of installed capacities and generation for all sources
    :param total_system_power: Total system power used in MWh
    :return: Combined LCOE in $/MWh
    """
    total_annual_cost = 0
    ...
    for source, params in energy_sources.items():
        if source in dynamic_values:
            ...
            annual_energy_used_mwh = dynamic_values[source]["annual_energy_used_mwh"]

            # Calculate parameters
            capacity_kw = installed_capacity_mw * 1000  # Convert MW to kW
            capex = params["capital_cost_per_kw"] * capacity_kw
            fixed_om = params["fixed_om_cost_per_kw_year"] * capacity_kw
            variable_om = params["variable_om_cost_per_mwh"] * annual_energy_used_mwh
            transmission_cost = params["transmission"] * annual_energy_used_mwh
            ...
            # Total annual cost for this source
            total_source_cost = annualized_capital_cost + fixed_om + variable_om + transmission_cost
            total_annual_cost += total_source_cost

        for source in dynamic_values:
            if dynamic_values[source]['annual_energy_used_mwh'] == 0:
                print(f"Warning: {source} has zero annual energy usage.")

    # Combined LCOE
    combined_lcoe = total_annual_cost / total_system_power
