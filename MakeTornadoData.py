...
BASE_ENERGY_SOURCES = deepcopy(energy_sources)
...
def reset_and_apply_capex_scenario(row):
    """
    Reset energy_sources to baseline, then apply CAPEX multipliers from
    tornadoData.csv capex_multipliers column.
    """
    global energy_sources
    global EV_Battery_Cost

    energy_sources = deepcopy(BASE_ENERGY_SOURCES)

    # Reset EV battery cost too, if it is part of scenario testing
    EV_Battery_Cost = 10000

    capex_raw = row.get("capex_multipliers", None)

    if pd.notna(capex_raw) and str(capex_raw).strip() not in ["", "{}", "nan"]:
        try:
            multipliers = json.loads(capex_raw)
        except json.JSONDecodeError:
            multipliers = ast.literal_eval(capex_raw)

        for source, multiplier in multipliers.items():
            multiplier = float(multiplier)

            if source.lower() in ["ev_battery_cost", "ev_battery"]:
                EV_Battery_Cost *= multiplier
                continue

            if source not in energy_sources:
                raise KeyError(f"{source} not found in energy_sources")

            if "capital_cost_per_kw" not in energy_sources[source]:
                raise KeyError(f"{source} has no capital_cost_per_kw field")

            energy_sources[source]["capital_cost_per_kw"] *= multiplier
...
def do_parallel_monte_carlo(
    ...
    if not battery_wear_calculated:
        temp_data = pd.read_csv("CAL_Temps_Minutely.csv")['Temperature'].values
...
# ============================================================
# RAW TORNADO DATA GENERATOR
# Replaces everything from: all_years_data = pd.DataFrame()
# ============================================================

SCENARIO_FILE = "TornadoRawSame.csv"
OUTPUT_FILE = "TornadoRawSameOut.csv"
...
def get_scenario_value(row, possible_names, default=None):
...
def apply_tornado_scenario(row):
    """
    Applies one row from tornadoData.csv to the global model state.
    CSV expected columns include:
    BL_installed_mw, wind_installed_mw, solar_installed_mw,
    Grid_Store_Max, factor_v2g, charge_rate, v2g_connection_cost
    """
    ...
    row = row.rename(lambda x: str(x).strip())

    Solar_Installed = float(row["solar_installed_mw"])
    Wind_Installed = float(row["wind_installed_mw"]) / 3200.0
    Baseload_Installed = float(row["BL_installed_mw"])
    Grid_Store_Max = float(row["Grid_Store_Max"])

    Factor_V2G = float(row["factor_v2g"])

    # Your current no-NG 2050 setup
    Factor_NSC = 0.15
    Factor_Smart_Charge = 1.0 - Factor_NSC

    V2G_Connect_Cost = float(row["v2g_connection_cost"])

    NG_Fact = 0.0
    NG_Installed = 0.0


def summarize_successful_runs(rows, value_column):
    values = [
        r[value_column]
        for r in rows
        if r.get("status") == "success" and pd.notna(r.get(value_column))
    ]

    if not values:
        return {
            f"{value_column}_mean": np.nan,
            f"{value_column}_std": np.nan,
            f"{value_column}_min": np.nan,
            f"{value_column}_max": np.nan,
        }

    return {
        f"{value_column}_mean": float(np.mean(values)),
        f"{value_column}_std": float(np.std(values, ddof=1)) if len(values) > 1 else 0.0,
        f"{value_column}_min": float(np.min(values)),
        f"{value_column}_max": float(np.max(values)),
    }


def run_single_scenario(row, scenario_index):
    scenario_name = str(get_scenario_value(
        row,
        ["scenario_name", "Scenario", "scenario", "name"],
        f"scenario_{scenario_index:03d}",
    ))

    scenario_cr = float(get_scenario_value(row, ["charge_rate", "CR", "cr"], SC_CR))
    scenario_cap = float(get_scenario_value(row, ["Cap", "cap"], Cap))
    scenario_connect_cost = float(
        get_scenario_value(row, ["v2g_connection_cost", "V2G_Connect_Cost"], V2G_Connect_Cost)
    )

    print("\n" + "=" * 80)
    print(f"RUNNING TORNADO SCENARIO: {scenario_name}")
    print("=" * 80)

    reset_and_apply_capex_scenario(row)
    apply_tornado_scenario(row)

    print("APPLIED CAPEX:")
    for source in ["solar", "wind", "nuclear", "battery", "V2G"]:
        if source in energy_sources:
            print(source, energy_sources[source].get("capital_cost_per_kw"))

    scenario_rows = []

    for run_idx in range(1, N_RUNS_PER_SCENARIO + 1):
        print(f"\nScenario {scenario_name}: run {run_idx}/{N_RUNS_PER_SCENARIO}")
        ...
        results = do_parallel_monte_carlo(
            ...
            V2G_Floor=V2G_Floor,
            Cap=scenario_cap,
            V2G_Connect_Cost=scenario_connect_cost,
        )
        ...
        output_row = {
            ...
            "V2G_Floor": V2G_Floor,
            "Cap": Cap,
            "pass_count": results.get("pass_count"),
            "min_pass_count": results.get("min_pass_count"),
        }

        if results["status"] == "success":
            carbon_lcoe = results["carbon_adjusted_lcoe_by_case"]

            output_row.update({
                "Combined_LCOE": results["combined_lcoe"],
                "Carbon_Adjusted_LCOE_SCC_51": carbon_lcoe.get("SCC_51"),
                "Carbon_Adjusted_LCOE_SCC_120": carbon_lcoe.get("SCC_120"),
                "Carbon_Adjusted_LCOE_SCC_190": carbon_lcoe.get("SCC_190"),
                "Carbon_Adjusted_LCOE_SCC_340": carbon_lcoe.get("SCC_340"),
            ...
            output_row["failure_reasons"] = " | ".join(
                sim.get("failure_reason", "")
                for sim in results.get("details", [])
                if sim.get("failed")
            )

        scenario_rows.append(output_row)
    ...
    for col in stats_columns:
        scenario_stats.update(summarize_successful_runs(scenario_rows, col))
    ...
    scenario_stats["successful_runs"] = len(successful_lcoes)
    scenario_stats["attempted_runs"] = N_RUNS_PER_SCENARIO
    scenario_stats["passing_lcoes_pipe"] = "|".join(f"{x:.6f}" for x in successful_lcoes)

    for r in scenario_rows:
        r.update(scenario_stats)
...
missing = required_input_columns - set(scenario_df.columns)

if missing:
    raise ValueError(
        f"{SCENARIO_FILE} is not the scenario input file. "
        f"Missing columns: {sorted(missing)}\n"
        f"Columns found: {list(scenario_df.columns)}"
    )

print("CSV COLUMNS READ BY PANDAS:")
print(list(scenario_df.columns))

all_raw_rows = []

for scenario_index, scenario_row in scenario_df.iterrows():
    scenario_results = run_single_scenario(
        row=scenario_row,
        scenario_index=scenario_index,
    )
    all_raw_rows.extend(scenario_results)

    # Incremental save in case the run is interrupted.
    pd.DataFrame(all_raw_rows).to_csv(OUTPUT_FILE, index=False)

raw_tornado_df = pd.DataFrame(all_raw_rows)
raw_tornado_df.to_csv(OUTPUT_FILE, index=False)

print("\n" + "=" * 80)
print(f"SAVED RAW TORNADO DATA TO: {OUTPUT_FILE}")
print("=" * 80)
print(raw_tornado_df.groupby("scenario_name")["Combined_LCOE"].agg(["count", "mean", "std", "min", "max"]))
